...
async function synthesizerToHTML(
  ...
  // Sort by importance: HTML/JS/TS first so the synthesizer sees actual logic
  // before config/readme files. This maximises the usefulness of the token budget.
  const filePriority = (p: string) => {
    if (/index\.(html|js|ts)$/i.test(p))  return 0;
    if (/\.(html)$/i.test(p))              return 1;
    if (/\.(js|ts)$/i.test(p))             return 2;
    if (/\.(css)$/i.test(p))               return 3;
    return 4; // README, json, etc.
  };
  const sortedEntries = Object.entries(merged)
    .sort(([a], [b]) => filePriority(a) - filePriority(b));
  const fileBlock = sortedEntries
    .slice(0, 40)
    .map(([p, c]) => `=== ${p} ===\n${c.slice(0, 6_000)}`)
    .join("\n\n---\n\n");
