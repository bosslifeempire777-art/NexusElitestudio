...
export default function NewProject() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createMutation.mutate(
      { data: { name, prompt: finalPrompt, type } as any },
      {
        onSuccess: (data) => {
              ...
              {type === 'game' && (
                <div className="pt-4 animate-in fade-in slide-in-from-top-4">
                  ...
                  <p className="text-xs text-muted-foreground font-mono mt-2">
                    Games run in the browser using HTML5 Canvas — fully playable in the preview.
                  </p>
                </div>
              )}
          ...
          <div className="flex justify-end space-x-4">
            ...
            >
              {createMutation.isPending ? (
                <>Initializing… <span className="w-4 h-4 ml-2 border-2 border-background border-t-transparent rounded-full animate-spin" /></>
              ) : (
                <>ENGAGE SWARM <PlaySquare className="ml-2 w-5 h-5" /></>
              )}
            </Button>
          </div>
        </form>
